...
@section('title', 'Register | DEV Dash')
...
@section('content')
    <div
        ...
        <div class="flex items-center justify-center flex-col gap-2 text-center mb-8">
            ...
            <h1 class="mt-2 text-2xl font-bold text-gray-800 dark:text-gray-200">Join the DEV Dash Community</h1>
      ...
      dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-[#30363d]">
        ...
        <form action="{{ route('register') }}" method="POST" x-data="{ loading: false }" @submit="loading = true">
            @csrf
            <x-input label="Name" type="text" name="name" placeholder="Enter your full name" />
            <x-input label="Username" type="text" name="username" placeholder="Enter your username" />
            ...
            <div class="text-xs text-gray-600 dark:text-gray-400 mt-2 text-center">
                ...
                <a href="#" class="text-primary-light dark:text-primary-dark hover:underline">Terms of Service</a>
                and
                <a href="#" class="text-primary-light dark:text-primary-dark hover:underline">Privacy Policy</a>.
            </div>
            ...
            <button type="submit" :disabled="loading" :class="{ 'opacity-50 cursor-not-allowed': loading }"
                ...
                <span x-show="!loading">Signup</span>
                <span x-cloak x-show="loading">Signing up...</span>
            </button>
        </form>

        <div class="mt-4 text-center text-sm text-gray-700 dark:text-gray-300">
            Already have an account?
            <a href="{{ route('login') }}" class="text-primary-light dark:text-primary-dark hover:underline">Login</a>
        </div>
    </div>
@endsection
